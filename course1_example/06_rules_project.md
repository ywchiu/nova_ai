...
把下面整份內容存到 `.clinerules/project.md`。

Cline 會讀取 `.clinerules/` 底下的 `.md` / `.txt` 檔案。不要存成 `.rules/project.md`，那是概念示意路徑，Cline 不會把它當成 Project Rules 自動載入。

開新對話時 AI 會自動讀，不需要每次重貼。
...
- Tests should map back to a specific acceptance criterion (mention the AC id in the docstring).
- Do not weaken tests just to make them pass.
- If a test fails, explain whether the issue is in the implementation, the test, or the spec — do not silently change tests.
...
When the task involves analyzing logs (test logs, simulation logs, build logs), use the `log-error-analysis` skill from `.cline/skills/log-error-analysis/SKILL.md` before parsing or summarizing logs.
