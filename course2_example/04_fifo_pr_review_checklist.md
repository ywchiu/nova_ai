...
這個例子是在看一個 PR：作者改了 FIFO 滿載時的判斷。對非 IC 背景的人來說，可以把它想成「原本排隊區滿了，只要再有人排隊就會亮警告；現在改成如果同時有人離開，就不亮警告」。

這種改動可能是合理的，也可能偷偷改掉系統規則。AI Review Agent 的任務不是說「可以 merge」，而是幫 reviewer 把風險列清楚：這是不是改了設計語意、有沒有補測試、有沒有同步更新文件、哪些地方一定要請設計或驗證負責人確認。
