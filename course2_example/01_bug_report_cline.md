...
這個例子可以想成：使用者只說「某個輸出偶爾變成 0，幫我修」。問題是，`0` 不一定永遠是錯；有可能是 reset 後的正常值，也可能是某個狀態下允許的輸出。

所以 AI 第一件事不應該是改 code，而是先整理「目前知道什麼、還缺什麼、要看哪些 log 或波形、需要補什麼 regression test」。這就像客服收到一句「系統偶爾壞掉」時，不能直接改後端，而是先問出可重現步驟。
