...
FIFO 可以想成一條排隊通道：資料先進來，之後照順序出去。`overflow` 就是隊伍已經滿了，還有人想再塞一筆資料進來。

問題在於「滿了還要寫入」不一定只有一種正確答案。有些系統會丟掉新資料，有些會暫停寫入，有些會亮一個錯誤旗標，有些允許同一個 cycle 先拿出一筆再放入一筆。所以當 Issue 只寫「overflow behavior 有問題」時，AI 不能直接決定怎麼修。這個範例要示範的是：先把模糊的一句話整理成可驗收條件和需要人確認的規格問題。
